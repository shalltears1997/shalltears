...
private:
  ...
  int  motion_id;
  DATA_xy sub_goal;
  DATA_xy tmp_goal;            // *** MOD ***  新增：tmp_goal 发送缓冲
...
public:
  connectWithMCClass(){};                         // コンストラクタ

  void connectLoopThreadBegin(bool *is_new);      // ループ受信スレッド開始
  void connectLoopThreadFinish();                 // ループ受信スレッド終了

  // データ受け渡し
  void dataInput (DataClass *data_in);
  void dataOutput(DataClass *data_in);

  ~connectWithMCClass();                          // デストラクタ
};
