...
class connectWithZPSClass
{
protected:

  // 内部変数
  int cri;
  const int NUM_OF_RECV_DATA = 33; //ZPSからの受信データ長 固定
  bool *zps_robot_new;  // 更新フラグへのポインタ
...
public:
  ...
  // 受信メソッド
  int receive();  // 一回きりの受信
  void receiveLoopThreadBegin(bool *zps_robot_new);  // ループ受信のスレッドを開始
